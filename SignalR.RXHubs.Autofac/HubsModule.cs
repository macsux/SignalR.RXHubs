...
namespace SignalR.RXHubs.Autofac
{
    public class HubsModule : Module
    {
        ...
        protected override void Load(ContainerBuilder builder)
        {
            ...
            var virtualHubTypes =
                _controllerAssemblies.SelectMany(asm => asm.GetExportedTypes().Where(type => !type.IsAbstract && typeof (IVirtualHub).IsAssignableFrom(type)));
            var normalHubTypes =
                _controllerAssemblies.SelectMany(
                    asm => asm.GetExportedTypes().Where(type => typeof (IHub).IsAssignableFrom(type) && !typeof (IVirtualHub).IsAssignableFrom(type)));
            ...
            foreach (var virtualHubType in virtualHubTypes)
            {
                ...
                builder.Register(context => (object) realHubFactory.Factory())
                    .As<IHub>()
                    .As(realHubFactory.HubType)
                    .ExternallyOwned();
            }
